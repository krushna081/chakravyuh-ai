...
export class OpenRouterProvider extends BaseProvider {
  ...
  constructor(config: ProviderConfig) {
    super(config, 'OPENROUTER_API_KEY', 'https://openrouter.ai/api/v1')
  }
  ...
  protected override async executeComplete(
    ...
    const response = await fetch(`${this.baseURL}/chat/completions`, {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://github.com/krushna081/chakravyuh-ai',
        'X-Title': 'Chakravyuh AI',
      },
  ...
  private mapResponse(data: Record<string, unknown>): CompletionResponse {
    ...
    if (!choicesData) {
      throw new ProviderError(
        'Missing choices in OpenRouter response',
        0,
        'server_error',
      )
    }
