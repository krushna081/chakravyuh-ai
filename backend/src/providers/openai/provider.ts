...
export class OpenAIProvider extends BaseProvider {
  ...
  constructor(config: ProviderConfig) {
    super(config, 'OPENAI_API_KEY', 'https://api.openai.com/v1')
  }
  ...
  private mapStreamChunk(
    ...
    if (choicesData.length === 0) return undefined

    const first = choicesData[0]!
    const delta = first.delta as Record<string, unknown> | undefined
    const finishReason = first.finish_reason as string | null
